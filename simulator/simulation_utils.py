...
def sleeping(simulation_context, viewport_window_list):
	"""
	...
	if carb.settings.get_settings().get("/rtx/rendermode") == "RayTracing":
		sleep_time = 0
		start = time.time()
		for _ in range(100):
			for vp in viewport_window_list:
				if vp.get_fps() == 0: continue
				sleep_time = max(1 / vp.get_fps() * 1.1, sleep_time)
			if sleep_time != 0 and time.time() - start > sleep_time * 2: # overly cautious
				break
			simulation_context.render()
	else:
		cnt = carb.settings.get_settings().get("/rtx/pathtracing/totalSpp")
		increase = carb.settings.get_settings().get("/rtx/pathtracing/spp")
		if cnt <= 0:
			cnt = 128
		while cnt >= 0:
			simulation_context.render()
...
def recorder_setup(_recorder_settings, out_path, enabled):
