	...
	if (num_obstacles > 0):
		...
		for n in range(num_obstacles):
			...
			if google_obs > 0:
				...
				ob_type = "shapenet"
				if shapenet is None:
					print("[INFO] Shapenet extension is not available. Resorting to google object.")
					ob_type = "google"
				elif loc == '':
			...
			try:
				...
				try:
					...
					continue
			if ob_type == "google":
				google_obs_used.append(shape_infos)
			elif ob_type == "shapenet":
				shapenet_obs_used.append(shape_infos)
