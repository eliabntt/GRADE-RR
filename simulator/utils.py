...
def position_object(environment, type: int, objects: list = [], ob_stl_paths: list = [], reset: bool = False, max_collisions: int = 200):
    """
    ...
    if environment.env_stl_path == None:
        print("No stl is being loaded for the environment, please pre-fix all objects locations or implement your own strategy")
    ...
    try:
        ...
        min_z = max(0.8, 0.8 + environment.env_limits[2]) if type == 0 else max(environment.env_limits[2], 0)
        max_z = max(environment.env_limits[2], 0) if (type == 1 or type == 3) else min(1.8 + environment.env_limits[2],
        ...
        return np.array(res.x) - environment.shifts[0], np.array(res.y) - environment.shifts[1], np.array(res.z) - environment.shifts[2], res.yaw
