...
def add_camera_and_viewport(path: str, resolution: list, old_h_ape, old_v_ape, is_headless=False, index=0, cam_index=0):
    """
    ...
    cam_index correspond to the n-th camera in the scene. We have ONE camera with TWO ros_bridges AND viewports associated to it.
    """
    ...
    camera_path = path + "/Camera"
    viewport = create_viewport(camera_path, is_headless, cam_index + index, resolution, old_h_ape, old_v_ape)
...
def create_viewport(camera_path, is_headless, index, resolution, old_h_ape, old_v_ape, npy=False):
    ...
    viewport_name = "Viewport" + (f" {index}" if str(index) != "0" else "") + ("_npy" if npy else "")
...
def ros_launchers_setup(roslaunch, env_limits_shifted, config, only_placement=False):
    ...
    if not only_placement:
        for i in range(config["num_robots"].get()):
            if not config["is_iRotate"].get():
                ...
                pass
    cli_args2 = ["collision_check", "collision_check.launch", "robot_mesh_path:={}".format(config["robot_mesh_path"].get())]
...
def pub_odom(robot_odom_frames, odom_pubs, _dc, meters_per_unit, diff_odom_frames = []):
    ...
    if len(diff_odom_frames) == 0:
        for index, handle in enumerate(robot_odom_frames):
            ...
            angles.append(Rotation.from_quat([odom.pose.pose.orientation.x,odom.pose.pose.orientation.y,odom.pose.pose.orientation.z,odom.pose.pose.orientation.w]).as_euler("XYZ"))
            odom_pubs[index].publish(odom)
    else:
        for index, handle in enumerate(robot_odom_frames):
                robot_body_ptr = _dc.get_rigid_body(handle)
                diff_body_ptr = _dc.get_rigid_body(diff_odom_frames[index])
                diff_handle = diff_odom_frames[index][1:] if diff_odom_frames[index].startswith("/") else diff_odom_frames[index]
...
def get_valid_robot_location(environment, first):
    ...
    # robot is nearly circular so I do not have to worry about collisions
...
def add_ros_components(robot_base_prim_path, n, ros_transform_components, ros_camera_list, viewport_window_list,
                       ...
                       dynamic_prims, config, imu_sensor, imu_props, old_h_ape, old_v_ape):
    """
    ...
    ros_transform_components.append(add_pose_tree(f"{robot_base_prim_path}{n}"))
    ...
    viewport_npy = create_viewport(f"{robot_base_prim_path}{n}/camera_link/Camera_npy", config["headless"].get(),
                                   n, config["npy_sensor_size"].get(), old_h_ape, old_v_ape, npy=True)
...
def move_robot(name: str, x: float, y: float, z: float, yaw: float, zlim: float, roll: float = 0, pitch: float = 0):
    """
    ...
    x, y, z: the position of the robot
    yaw: the orientation of the robot, in radians
    ...
    UsdPhysics.RevoluteJoint.Get(stage, name + '/roll_link/pitch_joint').GetLocalRot0Attr().Set(Gf.Quatf(quat.GetQuat()))
...
def add_robot_traj(path:str, config, meters_per_unit, time_codes_per_second):
    ...
    for entry in config["robot_traj"].get():
        add_translate_anim(path, Gf.Vec3d(entry["pose"]["x"] / meters_per_unit, entry["pose"]["y"] / meters_per_unit, entry["pose"]["z"] / meters_per_unit),
                                       entry["time"]*time_codes_per_second)
        add_rotation_anim(path, Gf.Vec3d(entry["pose"]["roll"], entry["pose"]["pitch"], entry["pose"]["yaw"]),entry["time"]*time_codes_per_second, use_double=True)
...
def check_pose_and_goals(init_loc, init_angle, c_pose, c_angle, path, goal_list, meters_per_unit, first):
    ...
    if not first and \
        (np.linalg.norm(np.array([goal_list[0][0],goal_list[0][1],goal_list[0][2]])-np.array(c_pose)+np.array(init_loc[0:3])) > 0.8 \
        or sum_dist > 0.6):
        return goal_list
    ...
    omni.kit.commands.execute('ChangeProperty',
            ...
            prev=0.0)
    omni.kit.commands.execute('ChangeProperty',
            prop_path=Sdf.Path(f'{path}/base_link/x_joint.drive:linear:physics:maxJointVelocity'),
            value=1000.0, # cm/s
            prev=0.0)
    omni.kit.commands.execute('ChangeProperty',
