...
try:
    ...
    launch_files = ros_launchers_setup(roslaunch, environment.env_limits_shifted, config, only_placement=True)
    ...
    if add_colliders(env_prim_path):
        simulation_context.play()
        x, y, z, yaw = position_object(environment, type=3, reset=True)
        environment.generate_map(out_dir, origin=[x[0], y[0], z[0]]) # NOTE: NOT PERFECT
        for _ in range(10):
            simulation_context.step()
    ...
    for n in range(config["num_robots"].get()):
        ...
        if config["init_loc"].get()["use"]:
            x = config["init_loc"].get()["x"]
            y = config["init_loc"].get()["y"]
            z = config["init_loc"].get()["z"]
            yaw = np.deg2rad(config["init_loc"].get()["yaw"])
            roll = np.deg2rad(config["init_loc"].get()["roll"])
            pitch = np.deg2rad(config["init_loc"].get()["pitch"])
        else:
        ...
        simulation_context.stop()
        move_robot(f"{robot_base_prim_path}{n}", x / meters_per_unit, y / meters_per_unit, z / meters_per_unit, yaw,
                   (environment.env_limits[5]) / meters_per_unit, roll, pitch)
        ...
        kit.update()
        if config["use_robot_traj"].get() and not config["autonomous"].get() and not config["use_joint_traj"].get():
    ...
    while n < rng.integers(0, 1 + max(0, config["num_humans"].get())):
        anim_len = 0
...
# IT IS OF CRUCIAL IMPORTANCE THAT AFTER THIS POINT THE RENDER GETS DONE WITH THE SLEEPING CALL! OTHERWISE PATH TRACING SPP WILL GET RUINED
    ...
    for _ in range(5):
        ...
        sleeping(simulation_context, viewport_window_list)
    ...
    goal_list = []
    if not config["use_robot_traj"].get() and not config["autonomous"].get() and config["use_joint_traj"].get():
