...
try:
	...
	if config["generate_map"].get() and add_colliders(env_prim_path):
		simulation_context.play()
	...
	for n in range(config["num_robots"].get()):
		...
		move_robot(f"{robot_base_prim_path}{n}", [x / meters_per_unit, y / meters_per_unit, z / meters_per_unit],
		           [roll, pitch, yaw], (environment.env_limits[5]) / meters_per_unit, config["is_iRotate"].get())
	...
	my_recorder = recorder_setup(config['_recorder_settings'].get(), out_dir_npy, config['record'].get(),1)
	...
	while kit.is_running():
		if can_start:
			if config['record'].get():
				if config["reload_references"].get():
		...
		if config["use_joint_traj"].get():
			if len(goal_list) > 0 and simulation_step >= 0:
				...
				goal_list = check_pose_and_goals(robot_init_loc[0], robot_init_ang[0], c_pose[0], c_angle[0], "/my_robot_0",
				                                 goal_list, meters_per_unit, simulation_step == 0)
				if len(goal_list) == 0:
					break
		...
		if simulation_step % ratio_camera == 0:
			...
			if simulation_step % ratio_camera == 0 and simulation_step / ratio_camera == experiment_length \
					and not config["neverending"].get() and goal_list.empty():
